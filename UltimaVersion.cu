...
__global__ void repartoComida(int* pos, int* food, int size, int columns, int* culture)
{
	int globalPos = threadIdx.x + blockIdx.x * blockDim.x;
	if(globalPos < size){
...
#include "cuda_check.h"
	...
	*/
	...
	cudaMemcpy( cellsDevice, cells, sizeof(Cell) * s ,cudaMemcpyHostToDevice );
	...
	cudaMemcpy( newCellsDevice, new_cells2, sizeof(Cell) * 1 ,cudaMemcpyHostToDevice);
...
for( iter=0; iter<max_iter && current_max_food <= max_food_int && num_cells_alive > 0; iter++ ) {
		...
		numeroBloq3=num_new_sources/tamanoBloq;
    	if (num_new_sources%tamanoBloq!=0) 
    	{
        	numeroBloq3++;
    	}
		...
		repartoComida<<<numeroBloq3, tamanoBloq, sizeof(int)*tamanoBloq>>>(numNewSourcesDevice, numNewSourcesDevice2, num_new_sources, columns, pDevice);

		if ( food_spot_active ) {

			num_new_sources = (int)(food_spot_size_rows * food_spot_size_cols * food_spot_density);

			numeroBloq4=num_new_sources/tamanoBloq;
    		if (num_new_sources%tamanoBloq!=0) 
    		{
        		numeroBloq4++;
    		}
			
			int* numNewSources4 = (int *)malloc( sizeof(int) * (size_t)num_new_sources);
			int* numNewSources5 = (int *)malloc( sizeof(int) * (size_t)num_new_sources);
			...
			cudaMemcpy( numNewSourcesDevice4, numNewSources4, sizeof(int) * num_new_sources ,cudaMemcpyHostToDevice );
			cudaMemcpy( numNewSourcesDevice5, numNewSources5, sizeof(int) * num_new_sources ,cudaMemcpyHostToDevice );
			//cudaMemcpy( numNewSourcesDevice6, numNewSources6, sizeof(int) * num_new_sources ,cudaMemcpyHostToDevice );
			repartoComidaFoodSpot<<<numeroBloq4, tamanoBloq, sizeof(int)*tamanoBloq>>>(numNewSourcesDevice4, numNewSourcesDevice5, num_new_sources, columns, pDevice);
			free(numNewSources4);
			free(numNewSources5);
			//free(numNewSources6);
		}
