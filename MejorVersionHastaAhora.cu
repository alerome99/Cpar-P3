...
#include "cuda_check.h"
    ...
    if(num_cells > (rows*columns)){
    	numeroBloq = 0;
    	numeroBloq=num_cells/tamanoBloq;
	...
	int* banderaDevice;
	Cell* newCellsDeviceTemp;
	Cell* newCellsDeviceTemp2;

	Statistics* statsDevice;
	int num_cellsAux3 = num_cellsAux2;
	if(num_cellsAux2 > 50000){
		num_cellsAux3 = 50000;
	}
	...
	cudaMalloc((void **) &newCellsDevice, sizeof(Cell) * (size_t)num_cellsAux3); //cells new cells
	...
	Cell *new_cells2 = (Cell *)malloc( sizeof(Cell) * num_cellsAux3 );
	if ( new_cells2 == NULL ) {
		fprintf(stderr,"-- Error allocating new cells structures for: %d cells\n", num_cells );
		exit( EXIT_FAILURE );
	}

	cudaMemcpy( newCellsDevice, new_cells2, sizeof(Cell) * num_cellsAux3 ,cudaMemcpyHostToDevice);
	...
	for( iter=0; iter<max_iter && current_max_food <= max_food_int && num_cells_alive > 0; iter++ ) {
		...
		*/
		...
		inicializarDevices<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(stepDeadCellsDevice, stepNewCellsDevice, aDevice, cDevice);
		...
		movimientoCelulas<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(cellsDevice, rows, columns, numAliveDevice, stepDeadCellsDevice, statsDevice, numeroCelulasDevice, pDevice, p2Device/*, foodShareDevice*/);
		...
		*/
		...
		comerComida<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(pDevice, cellsDevice, numeroCelulasDevice, rows, columns);
		...
		limpiarCelulas<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(cellsDevice, numeroCelulasDevice, aDevice, cellsDevice2);

		limpiarCelulas2<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(cellsDevice2, aDevice, cellsDevice, numeroCelulasDevice);
		...
		*/
		...
		anyadirCelulas<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(stepNewCellsDevice, newCellsDevice, cellsDevice2);
		...
		anyadirCelulas2<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(cellsDevice2, stepNewCellsDevice, numeroCelulasDevice, cellsDevice); //para unir el cellsDevice3 al cellsDevice
		//cudaCheckLast();

		actualizarNumCells<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(numeroCelulasDevice, stepNewCellsDevice);
		/*
		...
		*/
		...
		final<<<numeroBloq, tamanoBloq, sizeof(int)*tamanoBloq>>>(cDevice, stepNewCellsDevice, stepDeadCellsDevice, numAliveDevice, statsDevice);
